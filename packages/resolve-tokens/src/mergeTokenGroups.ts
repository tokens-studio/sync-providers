...
export function mergeTokenGroups(tokens: TokenSetsCombined, usedSets: UsedTokenSetsMap = {}, overallConfig: UsedTokenSetsMap = {}, activeTokenSet?: string): SingleToken[] {
  ...
  return tokenSetsOrder.reduce((mergedTokens, setName) => {
    ...
    setTokens.forEach((token) => {
      const existingIndex = mergedTokens.findIndex((t) => t.name === token.name as unknown as string);
